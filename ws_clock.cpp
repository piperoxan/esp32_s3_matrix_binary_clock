...
void Matrix_Init() {
  ...
  // Matrix.setTextColor(Matrix.Color(30, 30, 30));  //这个是像素字体的参数, 点阵无用
...
static void binary_pixel(uint8_t num, bool time[][2]) {
  ...
  for (int binary_pixel_last_index=5; binary_pixel_last_index > 0; binary_pixel_last_index--) {
    ...
    // }
    time[binary_pixel_last_index - 1][1] = num & 1;
    num = num >> 1;
